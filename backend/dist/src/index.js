try {
    ...
    handler.use(async (req, res, next) => {
        try {
            ...
            console.error("[FATAL][HANDLER] Erro não tratado:", err);
            res.writeHead(500, { "Content-Type": "application/json" });
            res.end(JSON.stringify({
                error: "Erro não tratado no handler",
                details: err.message,
                stack: err.stack,
            }));
        }
    ...
    console.error("[FATAL][GLOBAL] Erro ao inicializar handler:", err);
    module.exports = (req, res) => {
        res.statusCode = 500;
        res.setHeader("Content-Type", "application/json");
        res.end(JSON.stringify({
            error: "Erro fatal ao inicializar handler",
            details: err.message,
            stack: err.stack,
        }));
    };
